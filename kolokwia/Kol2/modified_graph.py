# Mateusz Wróbel
from kol2testy import runtests
from math import inf
from queue import PriorityQueue

#Rozwiazanie O(Elog(V))
#Modyfikacja grafu, aby nie mozna bylo wychodzic z resortu, jedynie zeby mozliwe bylo wchodzenie

def lets_roll(start_city, flights, resorts):

    _resorts = {}
    R = len(resorts)

    for r in resorts:
        # True - mozna uzyc
        # False - uzyty
        _resorts[r] = True
    cost = 0
    G = edges_to_graph(flights,_resorts)

    cost = modified_dijkstra(G, start_city, _resorts)

    return cost * 2


def modified_dijkstra(G, s, _resort):
    n = len(G)
    d = [inf for _ in range(n)]
    d[s] = 0
    q = PriorityQueue()
    q.put((0, s))  # time,v,
    cost = 0
    while not q.empty():
        time, v = q.get()

        if time > d[v]:
            continue
        # jesli jeszcze nie odwiedzilismy resortu
        if v in _resort and _resort[v]:
            _resort[v] = False
            cost+=time
            continue

        for u, w in G[v]:
            #nie przechodzimy przez resort znowu
            if d[u] > time + w:
                d[u] = time + w
                q.put((time + w, u))
    return cost


def edges_to_graph(E,_resorts):
    n = 0
    for u, v, w in E:
        n = max(u + 1, v + 1, n)
    G = [[] for _ in range(n)]
    for u, v, w in E:
        if u in _resorts:
            G[v].append((u,w))
        elif v in _resorts:
            G[u].append((v,w))
        else:
            G[u].append((v, w))
            G[v].append((u, w))
    return G


runtests(lets_roll, all_tests=True)
